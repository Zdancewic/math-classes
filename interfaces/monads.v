...
(* SAZ: If the monad operations can depend on the equivalence used
   for [A], they need to be parameterized by eqA.  Similarly, the
   bind operation should be proper.

   Without this added generality, it doesn't seem possible to prove
   that, for instance, the ret of the PM monad is proper.

   However, adding these additional parameters is a bit of a pain,
   since they have to be threaded through everywhere.

   Note:  in mathclasses, [Equiv A] is just the typeclass that
   carries the eq function (and it's written as [=]!!).
 *)
